 ...
 * @access  Private
 */
router.get('/telex/:telexId', catchAsync(async (req: Request, res: Response) => {
 ...
 * @access  Private
 */
router.get('/:id/medications', catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
...
router.get('/:id/adherence', catchAsync(async (req: Request, res: Response) => {
  ...
  const medications = await Medication.find({ userId: id, isActive: true });
  ...
  // Get adherence stats for each medication
  const medicationStats = [];
  let totalDoses = 0;
  let totalConfirmed = 0;

  for (const medication of medications) {
    const stats = await (Schedule as any).getAdherenceStats(medication._id, parseInt(days as string));
    const medicationData = stats.reduce((acc: any, stat: any) => {
      acc[stat._id] = stat.count;
      return acc;
    }, {});

    const medicationTotal = Object.values(medicationData).reduce((sum: number, count: any) => sum + count, 0);
    const medicationConfirmed = (medicationData as any).confirmed || 0;
    const adherenceRate = medicationTotal > 0 ? Math.round((medicationConfirmed / medicationTotal) * 100) : 0;

    totalDoses += medicationTotal;
    totalConfirmed += medicationConfirmed;
    ...
    medicationStats.push({
      ...
      adherenceRate,
      stats: {
        pending: (medicationData as any).pending || 0,
        confirmed: medicationConfirmed,
        missed: (medicationData as any).missed || 0
      },
      totalDoses: medicationTotal
    });
  }

  const overallAdherenceRate = totalDoses > 0 ? Math.round((totalConfirmed / totalDoses) * 100) : 0;
 ...
 * @access  Private
 */
router.post('/:id/reactivate', catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
